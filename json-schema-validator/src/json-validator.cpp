...
namespace {
    ...
    public:
        ...
        void set_target(std::shared_ptr<schema> target) {
            target_ = target;
        }
    };
...
namespace nlohmann {
    namespace json_schema {
        ...
        public:
            ...
            std::function<void(const std::string &, const std::string &)> &format_check() {
            ...
            std::shared_ptr<schema> get_or_create_ref(const json_uri &uri) {
                ...
                try {
                    ...
                    auto s = schema::make(subschema, this,{},
                    {
                        {
                            uri
                        }
                ...
                if (r != file.unresolved.end() && !(file.unresolved.key_comp()(uri.pointer(), r->first))) {
                    ...
                    return file.unresolved.insert(r,{uri.pointer(), std::make_shared<schema_ref>(uri.to_string(), this)})
                    ->second;
                }
            }
...
namespace nlohmann {
    namespace json_schema {
        ...
        void json_validator::validate(const std::string &msgString, basic_error_handler &err) {
            std::string errorMsg;
            try {
                json data = json::parse(msgString);
                root_->validate(data, err);
        ...
        void json_validator::set_root_schema(const json &schema, basic_error_handler &err) {
            std::string errorMsg;
            try {
                root_->set_root_schema(schema);
            } catch (const std::exception &e) {
        ...
        void json_validator::set_root_schema(const std::string &schemaString, basic_error_handler &err) {
            std::string errorMsg;
            try {
                json schema = json::parse(schemaString);
                root_->set_root_schema(schema);
            } catch (const std::exception &e) {
                errorMsg = (std::string)e.what();
                err.error("", "", errorMsg);
